...
critical("DATA-010", "Archives et références écartées protégées; droits informatifs", /mediaSelectionBlocked/.test(files.ui) && /Référence non diffusable/.test(files.ui) && /selected && mediaSelectionBlocked/.test(files.client) && /mediaChoiceAllowed/.test(files.rules), "Les droits à confirmer permettent le choix; les archives et références écartées pour d’autres motifs restent protégées.");
