...
critical("DATA-013", "Mutations métier et archives atomiques", ["setWorkflowStage", "setOpportunityStage", "setInternalProjectStage", "setEditorialDecision", "addMediaLink", "archiveMediaLink", "addCockpitFeedback", "upsertActionTask", "completeActionTask", "updateCockpitFeedbackStatus"].every((name) => new RegExp(`export async function ${name}\\([\\s\\S]{0,3200}writeBatch\\(db\\)[\\s\\S]{0,2400}batch\\.commit\\(\\)`).test(files.client)) && /setPersonalActionItemState[\s\S]{0,2400}transaction\.set\(archiveReference/.test(files.client), "La donnée métier et son historique doivent réussir ou échouer ensemble.");
