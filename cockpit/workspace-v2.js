...
export function mountWorkspace(api) {
  ...
  const pastMarkup = dateIso => isPastDate(dateIso) ? '<span class="v2-past-label" title="Date passée. La publication et ses validations restent consultables, sans changement de statut.">Passé</span>' : "";
  ...
  function enhanceGallery(card) {
    ...
    const show = (i, remember = true) => {
      ...
      if (remember && items[n]) state.galleryIndex.set(card.dataset.itemId, items[n].dataset.mediaId);
