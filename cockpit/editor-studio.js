import { fetchPublicationHistory, savePublicationContent } from "./firebase-client.js?v=20260718-b31";
import {
  ...
  weekForDate
} from "./publication-editor-schema.mjs?v=20260718-b31";

const runtime = { profile: null, getPosts: () => [], getRows: () => new Map(), button: null, panel: null, form: null, selectedId: "", revision: 0, returnFocus: null };
...
function loadDraft(item, { revision = null } = {}) {
  const row = rowFor(item.id);
  runtime.selectedId = item.id || "";
  ...
  setStatus(runtime.revision ? "Prêt à modifier. Rien n’est enregistré avant le bouton Enregistrer." : "Nouvelle publication non enregistrée.");
  updatePreview();
  refreshList();
}
...
function buildPanel() {
  ...
  panel.innerHTML = `<div class="studio-shell">
    ...
    <main class="studio-main"><header class="studio-heading"><div><h2>Éditeur de publication</h2><p>Les changements sont validés, versionnés et archivés; aucune suppression physique.</p></div><span class="studio-revision" data-studio-revision>Nouvelle</span></header>
      <form class="studio-form" data-studio-form><section class="studio-section"><h3>1 · Repères essentiels</h3><div class="studio-grid">
        ...
        <label class="studio-field">Identifiant stable<input data-field="id" maxlength="80" pattern="[A-Za-z0-9-]{3,80}" required></label>
  ...
  panel.querySelector("[data-studio-new]").addEventListener("click", () => loadDraft(blankDraft(), { revision: 0 }));
  panel.querySelector("[data-studio-duplicate]").addEventListener("click", () => {
    ...
    loadDraft({ ...source, id: publicationIdFrom({ title: `${source.title} copie`, dateIso }), title: `${source.title} — copie`, dateIso, week: weekForDate(dateIso, minimumDate()), originId: source.id, archivedEditorial: false }, { revision: 0 });
  });
...
async function saveForm(event) {
  ...
  const draft = formDraft();
  if (!draft.id) draft.id = publicationIdFrom(draft);
  ...
  try {
    const action = draft.archivedEditorial ? "publication classée sans suppression" : runtime.revision ? "publication modifiée dans le Studio" : "publication créée dans le Studio";
    const result = await savePublicationContent(draft, runtime.profile, { expectedRevision: runtime.revision, action });
    runtime.selectedId = result.id; runtime.revision = result.revision;
...
export function destroyPublicationStudio() {
  ...
  Object.assign(runtime, { profile:null, getPosts:()=>[], getRows:()=>new Map(), button:null, panel:null, form:null, selectedId:"", revision:0, returnFocus:null });
  document.documentElement.style.overflow = "";
}
