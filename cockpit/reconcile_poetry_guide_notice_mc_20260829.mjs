...
const ARCHIVE_ID = "poetry-guide-notice-mc-chronological-20260829";
...
if (notice.assigneeEmail !== "dg@bleumassawippi.com" || notice.audienceRole !== "director") {
  throw new Error("La révision ciblée doit rester dans la file personnelle de la direction.");
}

const configPath = path.join(os.homedir(), ".config", "configstore", "firebase-tools.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
if (!config.tokens?.access_token || Number(config.tokens.expires_at || 0) < Date.now() + 120_000) {
  throw new Error("Session Firebase CLI expirée; renouveler la session avant cette révision bornée.");
}
...
const desired = {
  ...
  updatedBy: "system_poetry_guide_mc_revision",
...
if (!alreadyApplied && !/21 pages|accueil est intégralement bilingue/i.test(String(before.message || ""))) {
  throw new Error("La notification a changé depuis la révision précédente; mise à jour automatique refusée.");
}
if (archiveResponse.ok && !alreadyApplied) {
  throw new Error("L’archive existe déjà alors que la notification diffère; intervention manuelle requise.");
}
...
console.log(JSON.stringify({
  ...
  changedFields,
  predecessorDetected: /21 pages|accueil est intégralement bilingue/i.test(String(before.message || "")),
...
const archiveFields = {
  ...
  entityId: ACTION_ID,
  action: "guide personnel du maître de cérémonie : ordre chronologique, Sanctuary intégrée et annonce des affiches QR",
  before: beforeTrace,
  after: afterTrace,
  actorUid: "system_poetry_guide_mc_revision",
  actorLabel: "Révision guide maître de cérémonie",
...
if (!commit.ok) throw new Error(`Révision Firestore refusée (${commit.status}) : ${JSON.stringify(commit.body).slice(0, 600)}`);

const verification = await request(`${base}/documents/actionItems/${ACTION_ID}`);
if (!verification.ok) throw new Error(`Relecture après révision refusée (${verification.status}).`);
...
if (verificationFailures.length || after.state !== before.state) {
  throw new Error(`Vérification après révision échouée : ${verificationFailures.join(", ") || "état personnel modifié"}.`);
}
console.log(JSON.stringify({
  applied: true,
  writes: 2,
