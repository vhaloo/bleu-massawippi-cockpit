...
import { notificationDecisionToken, notificationOwnerKey, notificationRecipientMatches, notificationSystemTag } from "./notification-recipient.js?v=20260907-b78";
...
function ensureStylesheet() {
  ...
  link.rel = "stylesheet";
  link.href = new URL("./view-mode.css?v=20260907-b78", import.meta.url).href;
...
function shouldUseCompleteView(target, type) {
  if (document.documentElement.dataset.workspace === "v2") return false;
...
export async function navigateToEntity({ type = "schedule", id = "", mediaId = "" } = {}) {
  ...
  window.dispatchEvent(new CustomEvent("cockpit:entity-will-open", { detail: { type: targetType, id: targetId, mediaId } }));
  if (targetType === "schedule" && document.documentElement.dataset.workspace !== "v2") prepareCalendarTarget(targetId);
...
function synchronizeCardDetails(card, expanded) {
  // The V2 reader owns each disclosure independently; the classic card's
  // remembered expansion must not reopen its history, forms and media details.
  if (card?.dataset.v2Prepared === "true") return;
